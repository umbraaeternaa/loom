...
Portable runtime quantity semantics are defined by [LOOM Portable Meter Frame v1](docs/meter_frame_v1.md); the reference interpreter enforces them while indirect generated-backend paths remain checker-closed. WASM and heap evolution are tracked separately in the [LOOM WASM Quantity Mediation Roadmap](docs/wasm_quantity_mediation.md), and `memory.grow` stays disabled until growth is explicitly metered.
