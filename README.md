...
An additive [`loom.trust.v2` receipt](docs/wasm_trust_provenance_v2.md) records
the checker-visible `roles`, `sub`, and `needs` policy clauses without changing
...
Source Equivalence result without changing earlier Gate schemas. The additive
...
`bounded-execution-required`; Bounded Execution v0 must remeasure again at spawn.
[`Bounded Execution v0`](docs/action_bounded_execution_v0.md) now consumes that
handoff exactly once: it requires a real OS network sandbox, remeasures and
launches through either an exact private snapshot or a fully checked root-owned
immutable macOS path, replaces the complete environment, supplies canonical
...
the Claim, and denies replay or further authorization.
[`Action Result Attestation v0`](docs/action_result_attestation_v0.md) composes
that terminal Result with the exact Compiler Receipt v4 and WASM artifact in a
canonical in-toto Statement v1. An external issuer signs DSSE PAE bytes; LOOM
holds no private key, verifies the signature before parsing, and grants no new
execution authority.
[`Multi-Action Plan v0`](docs/multi_action_plan_v0.md) composes existing Action
Capsules into a bounded closed DAG without creating a scheduler or plan-wide
authority. Every node preserves its exact Capsule and effect row behind a
non-shareable approval boundary; cycles, dependency drift, effect escalation,
...
[`Typed WASI Capability Mapping v0`](docs/typed_wasi_capability_mapping_v0.md)
then projects checked ABI v2 effect rows into a minimal versioned WIT import
set without building or authorizing an effectful Component. `IO/print` binds
only WASI stdout/streams, `Rand` binds only secure random, and `Alloc` remains
internal; `Net` and `FFI` fail closed until exact portable operation semantics
exist. Every mapping binds source, core bytes, trust receipts, exports, WIT,
WASI 0.2.8 spec digests, resource rules, and a non-executable lifecycle.
[`Effectful Component Adapter v1`](docs/effectful_component_adapter_v1.md)
then turns the closed `IO`/`Rand`/`Alloc` projection into a real one-shot
...
[`Effectful Component Host Execution v0`](docs/effectful_component_host_execution_v0.md)
then closes the spawn boundary: it re-verifies the complete signed Approval,
rebuilds the execution binding, remeasures Wasmtime and the Component, executes
...
repeating execution.
[`Effectful Component Execution Attestation v0`](docs/effectful_component_execution_attestation_v0.md)
then exports that complete terminal chain as a canonical externally signed
DSSE/in-toto Statement. It signs Result Binding, Host Execution, Action Result,
executed Component bytes, and Gate Receipt together, while explicitly claiming
neither independent attester quorum nor a SLSA level.
[`Portable Execution Evidence Bundle v0`](docs/effectful_component_execution_evidence_bundle_v0.md)
then packages that signature with exact source, WASM, compiler surfaces, nested
evidence, and public keys for bounded offline verification on another machine.
...
[`Evidence-fed Dogfooding v2`](docs/dogfooding_v2.md) removes the manual quorum
from that path. A fixed read-only adapter binds the exact canonical Git head and
successful CI run, a pinned operator key signs the exact review request, and
LOOM derives `(main 3)` only after all three facts are freshly reverified. The
result remains advisory: it grants no authority and executes no host action.
...
Start with the pinned first-run path in
...
release-check, the trust example, and the browser playground.
...
A third independent `windows-2025` lane builds a repository-owned native probe
twice with MSVC `/Brepro`, then exercises SID/DACL private custody,
component-by-component reparse-point refusal, a byte-identical private
...
The first read-only LOOM Gate contract is also available as
`loom.validate_manifest(value)`. It validates a closed task declaration,
normalizes set-like fields and absolute paths, and returns a deterministic
...
files, observed actions, and evidence into a deterministic
...
`loom.collect_ci_evidence(manifest, observation, run_id)` binds the observed
LOOM head to a successful public GitHub Actions run, its exact verify steps, and
the public `main` branch. It emits `syntax`, `citadel`, `docs-parity`, `fuzz`,
...
[`examples/operator_handoff_cli.py`](examples/operator_handoff_cli.py) is a
...
[`docs/gate_operator_key_storage.md`](docs/gate_operator_key_storage.md)
pins the storage boundary: the current reference issuer may read an external
...
pins Gate behavior: ordinary `read_paths` must not imply credential access,
secret-like reads require manifest-bound operator approval, secret-like reads
combined with outbound/reporting actions are rejected as possible exfiltration,
and receipts use dedicated `secret-lane` evidence that records approval or
blocking without raw paths or secret values. `loom-gate-manifest/v2` adds an
explicit closed `secret_access` lane for this protected request, but it remains
denial-first: the lane requires operator approval and still rejects
outbound/reporting combinations as possible `SecretExfil`. This is a protection
contract, not a capability to harvest passwords, keys, wallets, cookies, or
bank data.
