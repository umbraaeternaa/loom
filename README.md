...
kernel — small on purpose — and it is **self-verified by 437 checks** that the language can only ever
...
Portable runtime quantity semantics are defined by [LOOM Portable Meter Frame v1](docs/meter_frame_v1.md); the interpreter, generated Python/JavaScript backends, and WASM enforce them. WASM carries a private linked meter frame through named calls, closures/`applyN`, recursion, handlers, and FFI without adding host ABI obligations. The checker remains conservatively closed for indirect metered source paths until its quantitative analysis is deliberately relaxed. WASM and heap evolution are tracked separately in the [LOOM WASM Quantity Mediation Roadmap](docs/wasm_quantity_mediation.md), and `memory.grow` stays disabled until growth is explicitly metered.
