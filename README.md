...
interpreter. It is a research kernel — small on purpose — and it is **self-verified by 75
...
`with` reinterprets) · **capability seams for effect-opaque FFI** · **affine (use-once) seams** (whole-program, via a use-count lattice) · `if` / `let` · recursion ·
