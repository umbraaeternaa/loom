...
kernel — small on purpose — and it is **self-verified by 197 checks** that the language can only ever
grow *greener* (every new feature must keep them all passing).

```console
$ python3 run_tests.py
...
PASS — 197/197 citadel checks
```
...
Provenance also **flows through data**: [`examples/taint.loom`](examples/taint.loom) builds a value
from reviewed/audited inputs, binds it, computes with it — and `(trust 2 …)` still sees both anchors,
because the result carries the provenance it was made from.

```console
python3 loom.py run examples/taint.loom     # => 41  (the computed value still carries {human, audit})
```
