...
kernel — small on purpose — and it is **self-verified by 375 checks** that the language can only ever
grow *greener* (every new feature must keep them all passing).

```console
$ python3 run_tests.py
...
PASS — 375/375 citadel checks
```
...
first pure operations, `i31.add`, `i31.sub`, and `i31.mul`, preserve LOOM's tagged i31
