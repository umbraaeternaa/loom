...
`loom_multi_action.py` owns Multi-Action Plan v0, Execution State Machine v0,
Multi-Action Evidence Dataflow v0, and Multi-Action Byte Delivery Evidence v0.
It composes already closed
...
skips dependents. Its dataflow contract binds a direct source output digest to
an exact target Invocation Binding stdin digest and resolves only from a
successful signed Result. Byte Delivery Evidence additionally requires a
detached byte witness, checks canonical UTF-8 JSON plus exact digest and size,
and retains no raw payload. It transports no bytes and contains no live
scheduler, private key, ledger mutation, or host executor; it grants no
authority, does not claim process ingestion, and remains absent from the
browser bundle.
