...
def _check_effectful_component_result_binding() -> None:
    words = " ".join(EFFECTFUL_COMPONENT_RESULT_BINDING_DOC.read_text().split())
    required = (
        ...
        "loom-effectful-component-result-binding/v0",
        "loom-effectful-component-result-links/v0",
        "content-addressed join",
        "signed DSSE",
        '"authorization": "none"',
        '"remaining_evidence": []',
    ...
    if missing:
        raise SystemExit(
            "docs parity: Effectful Component Result Binding v0 drift: missing "
            + ", ".join(missing)
        )
    import loom as modular
    spec = importlib.util.spec_from_file_location("loom_effectful_result_standalone", DOCS_LOOM)
    if spec is None or spec.loader is None:
        raise SystemExit("docs parity: could not load standalone Result Binding boundary")
    standalone = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(standalone)
    names = (
        "build_effectful_component_result_binding_v0",
        "validate_effectful_component_result_binding_v0",
    )
    if any(not hasattr(modular, name) for name in names):
        raise SystemExit("docs parity: modular Effectful Component Result Binding v0 surface is incomplete")
    if any(hasattr(standalone, name) for name in names):
...
def _check_multi_action_plan_v0() -> None:
    ...
    if not MULTI_ACTION_BYTE_DELIVERY_DOC.is_file():
        raise SystemExit("docs parity: Multi-Action byte-delivery evidence contract is absent")
    delivery_words = " ".join(MULTI_ACTION_BYTE_DELIVERY_DOC.read_text().split())
    for needle in (
        "LOOM Multi-Action Byte Delivery Evidence v0",
        "loom.build_multi_action_byte_delivery_evidence_v0(",
        "loom.verify_multi_action_byte_delivery_evidence_v0(",
        "loom-multi-action-byte-delivery-evidence/v0",
        "detached byte witness",
...
def _check_windows_component_conformance_doc() -> None:
    if not WINDOWS_COMPONENT_DOC.is_file():
        raise SystemExit("docs parity: Windows Component Conformance v0 contract is absent")
    words = " ".join(WINDOWS_COMPONENT_DOC.read_text().split())
    for needle in (
        "LOOM Windows Component Conformance v0",
        "tools/windows_component_conformance.py",
        "loom-windows-component-ci-witness/v0",
        "windows-component-conformance-v0",
        "--offline --frozen --release",
        "authorization: none",
        "does **not** certify",
        "ACL/SID",
        "reparse-point",
        "Job Object",
        "verify-windows-component",
        "Federation v0",
    ):
        if needle not in words:
